# ============================================================
#  Christofides Solver — callable by case ID
#  Usage:  from christofides_solver import solve
#          cost = solve("S1")     # -> int
# ============================================================
import os
import math
...
# Optional: exact blossom matching via networkx.
# Falls back to greedy matching if not installed.
try:
...
def _odd_degree_vertices(edges, n):
    deg = [0] * n
    for u, v in edges:
        deg[u] += 1
        deg[v] += 1
...
def _min_weight_perfect_matching(odd_vertices, matrix):
    """Exact via blossom (networkx); greedy fallback otherwise."""
    if not odd_vertices:
        return []
    ...
    if HAS_NX:
        ...
        for i in range(m):
            ...
            for j in range(i + 1, m):
                v = odd_vertices[j]
                G.add_edge(u, v, weight=-int(matrix[u][v]))
        matching = nx.max_weight_matching(G, maxcardinality=True)
        return [(u, v) if u < v else (v, u) for u, v in matching]

    # Greedy fallback (not guaranteed 1.5-approx)
    m    = len(odd_vertices)
    used = [False] * m
    pairs = sorted(
        (int(matrix[odd_vertices[i]][odd_vertices[j]]), i, j)
    ...
    for _, i, j in pairs:
        if not used[i] and not used[j]:
            used[i] = used[j] = True
            edges.append((odd_vertices[i], odd_vertices[j]))
    return edges

# ---------------- Main callable ----------------
def solve(case_id):
    """
    Run Christofides on the generated instance `case_id`.
    ...
    -------
    int
        Cost (total length) of the Christofides tour.
    """
    matrix = _load_matrix(case_id)

    n         = len(matrix)
    mst_edges = _prim_mst(matrix)
    odd       = _odd_degree_vertices(mst_edges, n)
    matching  = _min_weight_perfect_matching(odd, matrix)
    adj       = _build_adjacency(n, mst_edges + matching)
...
def solve_with_details(case_id):
    ...
    Useful if you later want to log MST weight, matching weight, etc.
    """
    matrix    = _load_matrix(case_id)
    n         = len(matrix)
    mst_edges = _prim_mst(matrix)
    ...
    return {
        ...
        "matching_weight": sum(int(matrix[u][v]) for u, v in matching),
